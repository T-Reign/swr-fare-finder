...
if not all_stations:
    ...
    # Map layout labels to raw back-end codes seamlessly
    ticket_map = {
        f"{str(row['TICKET_TYPE_DESCRIPTION']).strip()} ({str(row['TICKET_CODE']).strip()})": str(row['TICKET_CODE']).strip()
        for _, row in ticket_data.iterrows()
        if not str(row['TICKET_CODE']).startswith(('1', '2'))
    }
    
    ticket_options = sorted(list(ticket_map.keys()))
    ...
    ticket_filter = [ticket_map[label] for label in selected_labels]

# --- 3. THE CALCULATION ENGINE ---
if origin and destination and ticket_filter:
    
    # 1. Determine the Baseline (Direct) Fare Pool
    if lock_baseline:
    ...
    # 2. FIND ALL DIRECT ROWS MATCHING CURRENT GEOGRAPHY
    direct_fare_rows = direct_df[(direct_df['ORIGIN_CLEAN'].str.strip().str.upper() == origin.strip().upper()) & 
                                 (direct_df['DEST_CLEAN'].str.strip().str.upper() == destination.strip().upper())]
    
    if direct_fare_rows.empty:
        st.warning(f"No direct fares found from {origin} to {destination} for your chosen filter criteria.")
    else:
        st.subheader(f"Direct Journeys & Product Mix Opportunities: {origin} to {destination}")

        # Master Line-of-Route Dictionary
        SEQUENCES = {
        ...
        # Loop over every valid through fare variation discovered
        for _, direct_row in direct_fare_rows.sort_values('FARE', ascending=True).iterrows():
            direct_fare = direct_row['FARE']
            target_ticket_code = direct_row['TICKET_CODE']
            route_desc = str(direct_row.get('ROUTE_DESCRIPTION', 'ANY PERMITTED')).strip().upper()
            ticket_desc = direct_row['TICKET_TYPE_DESCRIPTION']
            
            st.write(f"### Profile: {ticket_desc} ({target_ticket_code}) — Route: `{route_desc}` — **£{direct_fare:.2f}**")
            ...
            # 🌟🌟🌟 THE MIXED COMPOSITION ENGINE 🌟🌟🌟
            # This allows product-mixing walk-up fares with specific Advance tier codes
            if "ADVANCE" in ticket_desc.upper():
                filtered_df = df[(df['TICKET_CODE'] == target_ticket_code) | (df['TICKET_CODE'].isin(ticket_filter) & ~df['TICKET_TYPE_DESCRIPTION'].str.upper().str.contains("ADVANCE"))]
            else:
                filtered_df = df[df['TICKET_CODE'].isin(ticket_filter)]

            results = []

            # Evaluate options within the approved geographic tracks
            for split_station in valid_split_stations:
                l1_data = filtered_df[(filtered_df['ORIGIN_CLEAN'].str.strip().str.upper() == origin.strip().upper()) & 
                                     (filtered_df['DEST_CLEAN'].str.strip().str.upper() == split_station.strip().upper())]
                l2_data = filtered_df[(filtered_df['ORIGIN_CLEAN'].str.strip().str.upper() == split_station.strip().upper()) & 
                                     (filtered_df['DEST_CLEAN'].str.strip().str.upper() == destination.strip().upper())]
            ...
            if results:
                ...
                st.info(f"No cheaper combinations found undercutting this ticket profile tier ({target_ticket_code}).")
            st.write("---")
